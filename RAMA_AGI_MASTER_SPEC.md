...
## SECTION 71 — StockMind: the chart, and getting the engine's answers onto the screen

Task 6 of the six-part StockMind build.

### The page was not showing what the engine produced

Sections 64 through 70 made the engine honest. This section is about whether any of it
reached the screen, and mostly it did not.

`StockMind.jsx` read field names the engine does not emit:

```js
signal.strategy || signal.name || signal.algorithm   // engine emits `variant`
signal.direction === 'long'                          // engine emits `type: "LONG"`
```

So the **SETUP and DIR columns were permanently blank**, for every signal, always. And more
seriously: **entry, stop-loss and the three targets were never displayed at all.** The table
showed a probability and a grade with nothing to act on — the levels are the entire product
of a trading signal.

`basePrice` was also a free-text number the master typed. With a store holding 4,649 NIFTY
bars, that invites a signal priced off a number the market left days ago. It now comes from
the last stored close and is read-only.

### Decision — inline SVG, not recharts, and the exclusion was never a bug

`recharts@2.15.3` has been a dependency since the absorption and has never been imported.
Two things were assumed about that and both were wrong.

**First, the packaging.** `package.json`'s `build.files` contains
`"!node_modules/recharts/**"`, which reads like a bug that would stop the chart shipping. It
is not. That line sits alongside `!node_modules/react/**`, `!node_modules/react-dom/**` and
`!node_modules/zustand/**` — dependencies the renderer obviously uses. **Vite bundles the
renderer into `build/`**, so renderer dependencies are compiled into the bundle and never
need to exist in `node_modules` at runtime. Excluding recharts is exactly as correct as
excluding React. Nothing to fix.

**Second, whether to use it.** **Recharts has no candlestick chart.** Building one means a
custom `<Bar shape>` or a `Customized` component — writing the SVG by hand anyway — while
carrying roughly half a megabyte into the bundle and fighting its scale model to place five
horizontal level lines. The custom-shape work is identical either way, so the dependency
buys nothing. `PriceChart.jsx` is inline SVG: candlesticks, volume, price and date axes, a
crosshair readout, the entry zone as a band, and SL/entry/T1/T2/T3 as labelled lines.

One detail worth stating because getting it wrong is invisible: **the y-scale is computed
over the bars *and* the signal levels.** Scaling to the bars alone would clip a stop that
sits below the visible range, and the chart would then show a trade that appears to have no
risk.

`recharts` is now a genuinely unused dependency. Flagged for master rather than removed —
it is a `package.json` change with no functional benefit, and this build was verified with
it present.

### Decision — `market:` channels, not `stockmind:`

The `stockmind:` prefix is allowlisted in `preload.cjs` and unused, and it was tempting to
use it for the new surfaces. It was not used. All five existing channels are `market:*`
behind a single `marketIntel` bridge object, and adding a second prefix would split one
subsystem across two naming schemes for no gain. Ledger row 19 exists because nineteen
subsystems were once duplicated that way.

Thirteen channels added. Read-only surfaces (`ohlcv`, `inventory`, `news`, `news-coverage`,
`derivatives`, `option-chain`, `outcome-stats`, `models`, `scheduler-status`) are gated on
`stockmind.view`; actions that cost quota or change stored state (`news-sync`,
`resolve-outcomes`, `derivatives-sync`) on `stockmind.request`.

**Training is gated on `stockmind.config` (tier 2)** — the strictest existing StockMind
capability, not `request`. A retrain rewrites the artifacts every prediction then loads, so
it changes what the whole engine answers; that is a configuration change. **No new
capability was invented**: I8 says the matrix is defined once in
`shared/capabilities.json`, and an entry that fits was already there.

Every symbol is `encodeURIComponent`-ed into the path. That is not decoration — the symbol
comes from a text input, and `^NSEI` or a stray slash would otherwise change which route
is hit.

### The scheduler — because both loops were built and neither ran

Sections 68 and 70 each end with the same outstanding item: `/outcomes/resolve` and
`/news/sync` exist and **nothing calls them**. Without a schedule the learning loop never
resolves a claim, and the news series never gains a second day. Both features were complete
and inert. Master's standing instruction is that the installed app should take care of
itself, so it is closed here.

`marketIntel.startScheduler()` is armed from `main.cjs`'s registration list: outcome
resolution every 6 hours, news sync daily, first run 5 minutes after launch.

**It has no capability gate, deliberately, and that is not a hole.** `denyUnless` protects
the *renderer* boundary — it stops a low-tier UI caller reaching a privileged action. This
is main-process housekeeping with no session behind it, calling the same read-and-store
endpoints. **Fabricating a user object to satisfy a gate would be the actual hole**, and
inventing an identity is precisely what `server/index.cjs` was corrected for in Section 59.

Four constraints, each of which would be a defect if reversed:

- **It does not start the Python backend.** `ensureBackendRunning` would spawn Python on a
  timer even if master never opened StockMind — that is not housekeeping, it is a decision.
  Each tick checks whether the backend is *already* up and returns quietly if not. Verified:
  with the backend down, a tick makes **zero** HTTP calls.
- **It only touches read-and-store endpoints.** Never `/train`, never `/predict`, never
  `/backtest`. Asserted in the tests.
- **Arming is idempotent.** Two calls do not create two sets of timers, which would double
  every outcome's influence on the learned weights — Section 68's exactly-once `learnedAt`
  protects the records, not the schedule.
- **Timers are `unref`'d** so a pending tick can never hold the app open on quit, and every
  tick is wrapped so a throw cannot reach the event loop.

`RAMA_DISABLE_MARKET_SCHEDULER=1` turns it off, and `market:scheduler-status` reports
whether it is running and when each loop last ran.

### A stated limit that turned out not to apply

Sections 66 and 69 both record that `vite build` could not be verified in this workspace
because `node_modules` was not installed. **That is no longer true** — `node_modules` is
present and `vite build` completes in about 4 seconds. The renderer half of this task is
therefore genuinely verified rather than reviewed, which is a first for the UI work in this
project. The Electron main process still cannot be launched here, so the scheduler is
verified against stubs rather than in a running app.

### Verified

- `node --check` clean on `marketIntel.cjs`, `preload.cjs`, `main.cjs`.
- `npm run audit` clean: 35 store destructures and **89 bridge calls all resolve**, which is
  what proves the renderer's `window.rama.marketIntel.*` calls match `preload.cjs`.
- **`vite build` succeeds**, StockMind chunk 21.57 kB gzipped to 7.11 kB.
- **34 assertions on the scheduler** against a stubbed HTTP client and process manager:
  idempotent arming, the disable flag, zero HTTP calls when the backend is down, a
  running-but-unhealthy backend also declined, both ticks doing their work and being
  timestamped, never calling train/predict/backtest, all 18 channels registered, an
  unauthenticated caller refused, a viewer refused training, a master allowed, and symbol
  encoding (`^NSEI` → `%5ENSEI`, `a/b` → `A%2FB`).
- **The field contract, checked against a live `/predict`**: every one of the 27 keys the
  renderer now reads is present in the response — `MISSING: NONE`. Levels correctly ordered
  for a SHORT (`sl > entry > t1 > t2 > t3`), variants distinct, ids present, reasons
  non-empty.
- `/ohlcv` returns tail semantics (the last N bars, newest last), `high >= low` on every
  bar, dates ascending, and an unknown symbol yields an empty list with guidance rather
  than a 500.

### Not built

No intraday interval switching in the UI — the store holds daily bars and the interval
plumbing exists, but an intraday provider is a separate data question. No drawing tools, no
indicator overlays beyond the signal's own levels: an indicator drawn in the renderer would
be a second implementation of something `features.py` already computes, and the two would
drift. No option-chain table view; `market:option-chain` is wired and unused by the page.

---

## SECTION 72 — StockMind: historical news from GDELT, and whether it predicts anything

Master's instruction: use old data to train models on the impact of news, so the latest
analysis can be taken further.

*Written before implementing, per Section 28's working agreement.*

### Section 70's conclusion was correct but incomplete

Section 70 established that no RSS feed reaches back more than about sixteen days, and
concluded that news therefore could not be a trained feature. That conclusion was right
about RSS and wrong as a general claim, because it never tested a news *archive*.

**GDELT DOC 2.0 answers the question.** `https://api.gdeltproject.org/api/v2/doc/doc`, free,
no key, and it exposes three modes that matter here:

| Mode | Returns |
|---|---|
| `TimelineTone` | daily average tone for a query |
| `TimelineVol` | daily article volume — attention, which is a separate signal |
| `ArtList` | individual articles with title, domain, `seendate` |

**History floor is 2017-01-01**, verified: 2015-06 and 2016-06 both return the plain-text
body `Invalid query start date.` — note *plain text*, not JSON, so a parser that assumes JSON
on HTTP 200 will throw rather than report. That gives about nine years, roughly 2,250 trading
days, comfortably past the 800-day threshold `news.coverage()` asks for and the 150-row
holdout Section 69's gate requires.

### The finding that makes it worth building

Tone tracks market stress, measured before any of this was wired in:
...
Negativity more than doubled during the covid crash month and then recovered, and the single
most negative day in the window is **2020-02-28** — the first major global covid selloff.
A series that moves like that is a legitimate candidate feature. It is still only a
*candidate*: the Section 69 gate decides, unchanged.

### Decision — GDELT tone gets its own columns, never merged into `sentiment`

The existing `sentiment` column is the Section 70 lexicon applied to RSS headlines. GDELT
tone is a different measure, computed by someone else, over a different corpus, on a
different scale.

Writing both into one column would create a feature whose **meaning changes with its
source** — exactly the defect Section 67 removed when it stopped spot coming from
`UndrlygPric` on some dates and the OHLCV store on others. A model would learn the
2026-boundary where RSS collection began. So `gdelt_tone`, `gdelt_volume` and
`gdelt_articles` are separate columns with their own availability flag, and the two
sentiment measures can be compared rather than conflated.

### Decision — the access pattern is part of the design, not a detail

GDELT throttles hard on a burst. Probing produced `WinError 10054`,
`SSL: UNEXPECTED_EOF_WHILE_READING` and `RemoteProtocolError` in sequence, and — the part
that matters — **once a pooled keep-alive socket has been reset it stays poisoned for every
later request**, so a client that worked initially keeps failing afterwards.

What works, and is therefore mandatory:

- **A fresh client per call with `Connection: close`.** Not an optimisation to undo later; a
  shared pooled client is the failure.
- Roughly 6 seconds between calls, with 4 retries on an escalating backoff.
- A **year-sized window per call**, so nine years is nine calls rather than hundreds.
- Resumable and budgeted, the same shape as `derivatives.sync_history`: already-stored years
  are skipped, so re-running converges.

Also: OR'd terms **must** be parenthesised. `"a" OR "b"` returns the plain-text error
`Queries containing OR'd terms must be surrounded by ()`.

### Decision — tone is joined as-of, and lagged by construction

GDELT's daily tone for date D aggregates articles *seen* on D, which includes articles
published during and after that trading session. Joining tone(D) to a feature vector for
bar D would leak information from after the close into a prediction made at it.

So the join uses the **previous** available tone row, never the same date. That is the same
as-of discipline `featureset._deriv_row_for` uses, tightened by one day because news is
intraday-continuous while a bhavcopy is end-of-day.

This costs a day of freshness and removes a whole class of invisible lookahead. Section 66's
lookahead grade and Section 69's in-sample flag both exist because that class of error
produces excellent numbers that mean nothing.

### What gets measured

Once the series exists, `training.train(..., include_news=True)` runs it through the
**unchanged** gate: AUC ≥ 0.52, Brier skill > 0, mean fold AUC ≥ 0.50, holdout ≥ 150 rows,
and AUC clearing chance by two standard errors. Section 69 recorded that price features alone
produce no edge at any horizon from 1 to 20 bars; this asks whether adding news changes that.

The answer is reported either way. A news feature that fails the gate is a real finding about
news and Indian index direction, not a reason to loosen the gate.

### Built, and what could and could not be verified

`news.py` gained `gdelt_timeline`, `gdelt_query_for`, `backfill_history` and
`_merge_gdelt_rows`; `NEWS_COLUMNS` gained `gdelt_tone`, `gdelt_volume`, `gdelt_articles`;
`featureset.NEWS_FEATURES` gained six GDELT columns (`gdelt_tone`, `gdelt_tone_5d`,
`gdelt_tone_delta`, `gdelt_volume`, `gdelt_volume_5d`, `gdelt_available`), taking the
news-enabled vector to 113 columns. Route `POST /news/backfill`; channel
`market:news-backfill` on `stockmind.config`, since it runs for minutes against a
third-party API and materially changes what the models train on.

**The live nine-year pull could not be completed.** After the probing burst GDELT began
refusing this machine at the **TLS handshake** — `ConnectTimeout: handshake operation timed
out` — which is a throttle window, not a code fault. The source is verified working (181
daily tone points, and the covid tone series above); the backfill needs to run in a fresh
window. That is precisely what the resumable design is for, and `POST /news/backfill` can
simply be called again.

**So the honest position is: the source is proven, the pipeline is proven, the answer is not
yet in.** What was verified offline, with GDELT stubbed, is every part that is not the socket
— 60 assertions in `tests/test_histnews.py`.

The two that matter most: a tone series **constructed to predict** the next five bars
**passes** the unchanged gate (AUC 0.6715, Brier skill +0.0653, fold AUC 0.6894), and a
**random** tone series **fails** it (AUC 0.4576, Brier skill −0.0514) — same feature width,
same everything, only the data differing. That is what establishes the measurement can be
trusted when the real series arrives.

### Three defects found while building this

**1. `store.merge` replaced whole rows, so the two news sources erased each other.**
`drop_duplicates(subset=["date"], keep="last")` is right for bars — a re-fetch corrects every
field at once — and wrong for a series two writers populate different columns of. The RSS
write carried `gdelt_tone: None`, so writing RSS after a backfill **nulled the tone**, and the
claim "the columns are disjoint by design" was only true if the merge respected fields. Added
`combine=True`: a field-level merge where the newest **non-null** value wins per column, so a
newer null cannot erase an older value. Both news writers use it; bars are untouched.

**2. The backfill persisted only after every year had been fetched.** A run that hit its
budget or was interrupted kept **nothing** — the first attempt spent fifteen minutes on paced
requests and wrote zero rows. It now merges after each year, and since the store
de-duplicates on date, writing repeatedly converges.

**3. The pre-2017 floor check was unreachable.** It sat *after* the empty-window guard, and
for a pre-2017 year the window is already empty (start clamps up to the floor, end clamps
down to that December), so the loop `continue`d without recording why. A caller asking for
twelve years got an empty `skipped` list and no explanation. Floor check moved first.

Also tightened: the per-request timeout was 90 seconds with four retries, so one throttled
year could consume 900 seconds of a 900-second budget. Now 30 seconds and three attempts — a
refusal that arrives quickly is worth more than a response that might arrive eventually.

### The lookahead that was designed out

GDELT's tone for date D aggregates articles *seen* on D, which includes articles published
during and after that session. The join therefore takes the **strictly prior** row, never the
bar's own date, and the 5-day mean and delta are computed over prior days only. Verified with
a series whose tone equals its row index, so which row was picked is unambiguous: the feature
for 2026-01-20 (index 15) reads 14.0.

One day of freshness is a cheap price. Section 66's lookahead grade and Section 69's in-sample
flag both exist because that class of error produces excellent numbers that mean nothing.

---

## SECTION 73 — StockMind: three horizons, because master asked for all three

Master's answer to the question asked five times: intraday **and** swing **and** positional.
That is not a choice of one horizon; it changes the model layer from a single fitted default
into a set.

*Written before implementing, per Section 28's working agreement.*

### Decision — a horizon is `(interval, bars)`, not a number

`DEFAULT_HORIZON = 5` was five *daily* bars. "Intraday" cannot be expressed that way at all:
on daily bars the smallest possible horizon is one day, which is by definition not intraday.

So a horizon is a pair — the bar interval and how many of those bars ahead the label looks:

| Name | Interval | Bars ahead | Meaning |
|---|---|---|---|
| `intraday` | `60m` | 3 | ~3 hours, inside one session |
| `swing` | `1d` | 5 | about a week |
| `positional` | `1d` | 20 | about a month |

### Decision — intraday means hourly bars, because that is the only depth that exists

Probed against Yahoo, per symbol, verified:

| Interval | Max range | Bars | Span |
|---|---|---|---|
| `1m` | 5d | 1,876 | 4.3 days |
| `5m` | 1mo (`3mo` → **HTTP 422**) | 1,726 | 30 days |
| `15m` | 1mo (`3mo` → **HTTP 422**) | 576 | 30 days |
| `30m` | `3mo` → **HTTP 422** | — | — |
| **`60m`** | **2y** | **3,499** | **729 days** |
| `1d` | 5y+ | 1,240+ | years |

Bars-per-session cross-checks against the NSE's 6h15m day: 5m gave 82/day (expect ~75), 15m
27/day (expect ~25), 60m 6.8/day (expect ~7). Real session data, not padding.

**Only `60m` has the depth Section 69's gate requires.** 3,499 bars at stride 2 is roughly
1,600 rows and a ~320-row holdout, comfortably over the 150 floor. Five- and fifteen-minute
bars cap out at thirty days: the holdout would fall under the floor, and thirty days is a
single market regime, so a model fitted on it would be measuring one month's weather. They
remain fetchable for display; they are not trainable, and saying so is better than shipping a
model whose validation cannot mean anything.

### Decision — the three answers are reported separately, never averaged

The tempting move is to blend the three into one headline probability. That is rejected.

A three-hour call and a one-month call are **claims about different questions**. Averaging
them produces a number that describes neither, and it would hide the case that matters most
to a trader: **when the horizons disagree.** Intraday bearish against positional bullish is
not noise to be smoothed away — it is the signature of a pullback inside an uptrend, and it
is exactly the information that decides whether to hold, trim, or wait.

So `/predict/multi` returns one block per horizon plus an explicit agreement reading, and
Section 76's justification bullets will say which horizons agree and which do not.

### Decision — artifacts become horizon-scoped, with the old names still honoured

`rf_direction.pkl` becomes `rf_direction__1d_h5.pkl`, `rf_direction__60m_h3.pkl` and so on.

An unsuffixed legacy artifact is still loaded as the default when no horizon-scoped file
exists, so an install that trained before this change keeps working (I11). The feature
**manifest stays single** — feature names come from one builder and do not depend on the
horizon — but **training provenance becomes per-horizon**, since train range, holdout range
and the gate verdict are all horizon-specific and conflating them would let a positional
model's evidence vouch for an intraday one.

### Two defects this exposed before a line of it was written

**1. `store.merge` would have destroyed every intraday series.** It writes with
`date_format="%Y-%m-%d"`. Every bar in a session would serialise to the same midnight
timestamp, and the de-duplication on `date` would then keep **one bar per day** — a 3,499-bar
hourly series silently reduced to ~500 daily rows, with no error anywhere. Fixed by writing
the time component whenever the series is not daily.

**2. `providers._fetch_yahoo` hardcodes `interval=1d`.** No intraday fetch was possible at
all. It now takes an interval, and uses Yahoo's `range=` form for intraday rather than
explicit epochs — the intraday windows are range-limited server-side and `range` is what
those limits are documented against. Daily keeps the explicit `period1`/`period2` epochs,
because Section 65 established that `range=max&interval=1d` silently downsamples to monthly.

### Not changed

`DEFAULT_HORIZON` stays 5 daily bars for the existing single-horizon path, so every current
caller behaves identically. Section 69's gate is untouched — three horizons means three
verdicts from the same criteria, not a softer bar for the harder question.

### Verified

`ai_backend/engine/horizons.py` is new; `training.py` gained `train_horizons` and
horizon-scoped artifact/provenance paths; `registry.ensemble_predict` takes a `horizon_key`
and prefers that horizon's own fitted pipeline per member, falling through to the shared model
for any member without one; `store.py` gained `is_intraday`; `providers.py` gained
`INTRADAY_RANGE` and an interval argument. Routes `GET /horizons`,
`GET /predict/multi/{symbol}`, `POST /train/horizons`; channels `market:horizons`,
`market:predict-multi` on `stockmind.view` and `market:train-horizons` on `stockmind.config`.

**The intraday store defect, verified against real data.** A live `60m` sync of NIFTY 50
returned **3,448 hourly bars, 2024-08-29 → 2026-08-28, across 494 sessions — exactly 7.0 bars
per session**, which is what a 6h15m NSE day gives in hourly buckets. All 3,448 timestamps are
distinct, the round-trip through CSV preserves every one, and the first row on disk reads
`2024-08-29 03:45:00`. **Before the fix those 3,448 bars would have become 494**, because
every bar in a session serialised to midnight and the de-duplication kept one. A `15m` sync
returned 576 bars at exactly 25.0 per session, matching Yahoo's one-month cap. Daily is
untouched: 4,649 bars, still written date-only as `2007-09-17`.

**100 assertions in `tests/test_horizons.py`**, the load-bearing ones being: 70 synthetic
hourly bars survive a store round-trip with distinct closes and preserved intra-session order
while a daily file still serialises date-only; swing and positional produce **different**
artifact names despite sharing an interval; `None` yields the legacy unsuffixed name so a
pre-Section-73 install still loads; a permuted feature manifest **refuses** the horizon
artifact and the prediction still answers from the heuristic ensemble; a horizon with its own
artifact reports `horizonModels: ["random_forest"]` while the shared call reports `[]`; and the
agreement block emits **no** blended probability at all, with cross-horizon conviction taken as
the **weakest** rather than the mean, because the strongest reading cannot vouch for the others.

`npm run audit` clean; `vite build` succeeds; `node --check` clean on both `.cjs`.
**878 assertions across nine suites.**
...
## SECTION 75 — StockMind: alerts, and what entitles one to tell master to act

Master's words: *"tracked, updated/intimated to user when to leave, buy more/sell more at any
point of time"* and *"protect the user/master's capital"*.

This is the section where StockMind could do the most damage. Section 73 measured, on real
data, that **no horizon's model clears the gate** — the positional model looked best and was
refused for taking credit for the market's drift. An alert engine that turns those same
probabilities into "EXIT NOW" would launder a failed model into an instruction, which is
precisely what Section 69's gate was built to prevent, and it would do it at the moment master
has money on the table. So the first decision is not which alerts to emit. It is what gives an
alert the right to be acted on.

### Three evidence classes, and only two of them can act

Every alert carries an `evidence` field, and it is the load-bearing part of the design:

| Class | Meaning | Can it be acted on? |
|---|---|---|
| `DECLARED` | Master's own thesis — his stop, his target | **Yes.** Reporting a stop breach is reporting a fact against his own stated instruction. Rāma is not forecasting; it is watching the level he set. |
| `MEASURED` | Arithmetic over stored bars and his fills — drawdown from peak, days held against the thesis horizon, concentration, no stop recorded | **Yes.** These are counting, not predicting. |
| `MODEL` | A directional probability from `predict_all` | **Only if that horizon's model cleared the gate.** None do, so today every one of these is emitted with `actionable: false` and the gate's refusal reason attached. |

A `MODEL` alert on an ungated horizon is **neither silently dropped nor silently promoted**.
Dropping it hides that Rāma looked; promoting it is the capital-destroying failure. It is
returned, marked non-actionable, with `whyNotActionable` naming the horizon and quoting the
gate. `alerts.model_entitlement()` reads the per-horizon provenance from Section 73 and reports
`entitled: false` plus the recorded reason, so the claim is checked against what was actually
measured rather than assumed.

### The asymmetry: protecting capital and risking more of it are not the same act

**Exit and reduce can be justified by arithmetic alone. Adding cannot.**

A stop breach, a 15% drawdown or a 40% concentration is a fact about risk already taken, and
acting on it *reduces* exposure. The worst case of a false positive is master exits a position
that would have recovered — he loses upside. Adding is the opposite: it *increases* exposure,
and the worst case of a false positive is a larger loss on a thesis that was already wrong.

So `ADD` exists as a kind, but it is reachable **only** through the `MODEL` class, which means
it is currently never actionable. Rāma will not tell master to put more money in on the
strength of arithmetic that cannot see the future. This asymmetry is deliberate and is not an
oversight to be tidied up later.

### A stale price disqualifies every alert that compares a price

Section 74 built `priceStale` for exactly this. If the newest stored bar is older than the
newest fill, or older than `MAX_PRICE_AGE_DAYS`, then every price-dependent alert is marked
non-actionable and a single `PRICE_STALE` alert explains why. An exit alert fired on last
week's close is worse than no alert, because it carries the authority of a system that looks
current. Rejected suppressing them silently: master must be able to see that the reason he has
no guidance is a missing sync, which is fixable.

### Alerts are computed fresh, never stored

Rejected persisting fired alerts with an "already notified" flag. A stored alert is a claim
about the present that ages badly — the position moves, the price updates, the thesis is
revised, and the stored copy still says EXIT. Every call recomputes from the current ledger and
the current bars. De-duplicating notifications so master is not pestered is a UI concern and
belongs where the notification is shown, not in the evidence.

### The rules

Thresholds are module constants, each with the reason it has that value:

| Kind | Evidence | Action | Severity | Trigger |
|---|---|---|---|---|
| `STOP_BREACHED` | DECLARED | EXIT | critical | long: `last <= stop`; short: `last >= stop` |
| `STOP_APPROACHING` | DECLARED | REVIEW | warning | within `STOP_PROXIMITY_PCT` (2%) of the stop |
| `TARGET_REACHED` | DECLARED | REDUCE | warning | long: `last >= target`; short: `last <= target` |
| `TARGET_APPROACHING` | DECLARED | REVIEW | info | within `TARGET_PROXIMITY_PCT` (2%) |
| `NO_STOP_SET` | MEASURED | REVIEW | warning | an open position with no `stopPrice` — the loss is bounded by nothing Rāma can check |
| `DRAWDOWN_FROM_PEAK` | MEASURED | REDUCE | warning | peak close since the first fill down `DRAWDOWN_PCT` (15%) |
| `LOSS_THRESHOLD` | MEASURED | REVIEW | warning | unrealised P&L at or below `−LOSS_REVIEW_PCT` (10%) |
| `THESIS_EXPIRED` | MEASURED | REVIEW | warning | held longer than the thesis horizon's span × `THESIS_HORIZON_TOLERANCE` (2.0) |
| `CONCENTRATION` | MEASURED | REDUCE | warning | one position above `CONCENTRATION_PCT` (40%) of invested capital |
| `EQUITY_SHORT` | MEASURED | REVIEW | critical | net short on cash equity, which is not deliverable in India |
| `PRICE_STALE` | MEASURED | NONE | info | the price could not be trusted, so nothing else can be |
| `MODEL_AGAINST_POSITION` | MODEL | EXIT/REDUCE | — | a horizon points against the position |
| `MODEL_SPLIT` | MODEL | REVIEW | — | the horizons disagree (Section 73's `agreement`) |
| `MODEL_SUPPORTS_ADD` | MODEL | ADD | — | horizons aligned with the position at conviction |

`THESIS_EXPIRED` is the one worth calling out, because it is the alert master did not ask for
and probably needs most. A position opened on a **swing** thesis — five trading days — and
still held four months later is no longer the trade he decided on; the reason has expired even
if the price has not moved. Rāma converts the thesis horizon's bars into calendar days
(`TRADING_TO_CALENDAR = 7/5`) and says so.

### API and wiring

`alerts.evaluate_position(view, book_invested, prediction, entitlement)` is the pure function;
`alerts.evaluate(symbol=None, include_prediction=False)` walks the ledger, and
`alerts.model_entitlement(horizon)` answers whether a horizon may speak. Route
`GET /alerts` (+ `?symbol=`, `?includePrediction=`), channel `market:alerts` on
**`stockmind.view`** — reading a warning about one's own position is not a privileged action,
and gating protection behind a higher tier would be the wrong way round.

### Verification

**1,176 assertions across eleven suites**, `tests/test_alerts.py` contributing **127**. The
suite is written around what must *not* happen, because a test that only checked alerts appear
would pass on a module that shipped a failed model's opinion as an instruction:

- a `MODEL` alert is **non-actionable** while no horizon has cleared the gate, and its
  `whyNotActionable` **quotes the gate** rather than paraphrasing it
- the same alert **becomes actionable** when an entitlement is supplied — so the gate is doing
  the work, not a hardcoded refusal
- and a gate pass **does not survive a stale price**
- `ADD` never appears with `DECLARED` or `MEASURED` evidence in **any** scenario tested
- a stale price leaves `STOP_BREACHED` **reported but not actionable**, while `NO_STOP_SET`
  stays actionable because it compares no price at all
- the stop comparison **flips with the sign**: a short above its stop is breached
- `THESIS_EXPIRED` fires at 40 days on a swing thesis and **not** on a positional one
- the empty-ledger summary says "nothing to warn about", and the with-predictions summary says
  "no horizon's model has cleared the gate" — master can tell the two apart

**Two real defects found by these tests.**

1. **`model_entitlement` could have vouched for a model belonging to no horizon.**
   `horizons.get()` returns `None` for a name it does not know, and `training.load_provenance(None)`
   reads the **legacy unsuffixed** report — so a mistyped horizon would have been answered from
   that file, and could have returned `entitled: True`. An unknown name now refuses explicitly
   and says the legacy record must not answer for it.
2. **The stale-price reason discarded the useful message.** The flag match was narrow enough to
   miss the ledger's own note — *"the newest stored bar is 88 days old, sync before trusting
   P&L"* — and fell back to generic text. The ledger's note tells master what to *do*; the
   generic line does not. It is now preferred, with the generic text only as a fallback.

`node --check` clean on both `.cjs`, `py_compile` clean, `npm run audit` clean, `vite build`
succeeds in 5.70s.

### What this does not yet give master

The bridge methods exist and resolve but **no UI panel calls them**, so alerts are reachable by
API only — the same gap Section 74 left. Nothing pushes an alert at master either: `GET /alerts`
must be asked. A `STOP_BREACHED` on a position he is not looking at will not reach him until
something polls, and wiring that into the Section 71 scheduler is a follow-up.

The honest summary of where this leaves the feature: **the protective half works and the
predictive half is withheld.** Stop breaches, drawdowns, concentration, expired theses and
missing stops are all real, checkable and actionable today. Every alert that would require
knowing where the price is going is present, marked non-actionable, and carrying the reason —
and it will stay that way until a horizon earns its way through the gate.

---

## SECTION 76 — StockMind: justification, and the difference between an observation and a claim

Master's words: *"providing simple bullet points to justify the prediction and also intimate
warnings or pitfalls or even traps to protect the user/master's capitals"*.

### The trap this section is built to avoid

"Justify the prediction" is a dangerous instruction to follow literally when the prediction has
not earned trust. Section 73 measured that no horizon clears the gate. A bullet list of
supportive-sounding technical observations printed underneath an ungated probability is the most
harmful artefact in this whole codebase: **it makes a coin flip look researched.** The
observations would all be true, the probability would be worthless, and the juxtaposition would
imply the first supports the second.

So a justification here explains **what the number rests on**, and when it rests on nothing
validated, the *first* bullet says exactly that.

### Observation, convention, forecast — never merged

Every bullet carries a `basis`, and this is the load-bearing distinction:

| `basis` | Meaning | Example |
|---|---|---|
| `observation` | A measured fact about stored data | "RSI(14) is 72.4" |
| `convention` | How the market conventionally reads that fact — widely used, **not validated here** | "Above 70 is conventionally read as overbought" |
| `forecast` | A model's output | "P(up over 5 days) = 0.58" |
| `gate` | Whether that forecast is entitled to be believed | "The swing model has not cleared the gate" |

`"RSI is 72.4"` is a fact. `"RSI is 72.4 therefore it will fall"` is a claim StockMind has not
earned. They are never combined into one sentence. Rejected the usual approach of writing
bullets like "Overbought — expect a pullback": it reads as analysis and is actually an untested
assertion wearing analysis's clothes.

Evidence classes are **reused verbatim from Section 75** (`DECLARED` / `MEASURED` / `MODEL`)
rather than a second vocabulary being invented. A reader who has learned what `MEASURED` means
on an alert must not have to learn a different scale for a bullet.

### Warnings come from data that exists, and absence is itself reported

Master named the pitfalls he wanted: correlated positions, low delivery percentage, expiry
pinning, event risk, illiquidity, drawdown breach, confidence collapse. Every one is built on a
field that is really there — checked before designing, not assumed:

| Warning | Source field | Real? |
|---|---|---|
| `EXPIRY_PINNING` | `days_to_expiry`, `max_pain_dist`, `oi_concentration` | stored, backtestable (S67) |
| `EXPIRY_IMMINENT` | `days_to_expiry` | stored |
| `PCR_EXTREME` | `pcr_oi` | stored |
| `WIDE_STRADDLE` | `straddle_pct` | stored |
| `BASIS_DISLOCATION` | `fut_basis_pct` | stored |
| `WEAK_ROLLOVER` | `rollover_pct` | stored |
| `LOW_DELIVERY` | `delivery_data().deliveryPct` | live fetch only |
| `ILLIQUID` | bar `volume` vs its own average | stored |
| `GAP_RISK` | bar-to-bar opens vs prior closes | stored |
| `EVENT_RISK` | `news.classify_event` over headlines | live fetch only |
| `CORRELATED_POSITIONS` | **return correlation computed from stored bars** | stored |
| `CONFIDENCE_COLLAPSE` | `uncertainty` / `epistemic` / `suppressed` | model output, `MODEL` class |

**Correlation is measured, not assumed from a sector table.** Rejected hardcoding a
symbol→sector map to decide which positions are related: such a map would be my guess, would
rot, and would miss the real cases — an index and one of its own heavyweight constituents are
correlated by construction, not by sector label. Pairwise Pearson correlation of daily returns
over a stored lookback catches that automatically and is checkable by master against the same
bars. It also means the warning improves as the store deepens, with no taxonomy to maintain.

**A check that could not run produces a warning saying so.** `DERIVATIVES_ABSENT`,
`DERIVATIVES_STALE`, `CORRELATION_INSUFFICIENT`. This is Section 75's rule again: master must
never read silence as safety. An empty warning list has to mean "checked and clear", never
"could not look".

### Live fetches are opt-in

`LOW_DELIVERY` and `EVENT_RISK` need the network. They sit behind `include_live=False` by
default, because a justification that silently takes seconds and can fail is worse than one
that states which two checks it skipped. When skipped, they are reported as skipped.

### API and wiring

`explain.justify(symbol, exchange, prediction, include_live)` → bullets;
`explain.warnings_for(...)` → warnings; `explain.correlations(symbols, exchange, lookback)`;
`explain.brief(...)` → bullets + warnings + entitlement + a caveat line, which is the one call
a UI needs. Route `GET /explain/{symbol}`, channel `market:explain` on **`stockmind.view`**,
same reasoning as Section 75: understanding why must not cost a higher tier than being told what.

### Verification

**1,277 assertions across twelve suites**, `tests/test_explain.py` contributing **101**. Written
around restraint rather than coverage, because a suite that only checked bullets are produced
would pass on a module printing *"Overbought — expect a pullback"* under an unvalidated
probability, which is the exact harm:

- a high RSI yields **two** bullets — one `observation` stating only the number, one
  `convention` that names itself a convention **and** says it is not a measured edge
- the observation bullet does not contain the word "overbought" at all
- no bullet in any scenario contains "therefore", "will rise", "will fall" or "expect a"
- **the `gate` bullet is emitted at a lower index than the `forecast` bullet** it qualifies
- the gate bullet says the measured bullets are *"rather than support for it"*
- `caveat` is asserted present in the payload, so bullets cannot be rendered without it
- every "could not run" case carries `checked: false`: `DERIVATIVES_ABSENT` (which states in
  words that an empty list is not "clear"), `LIQUIDITY_UNCHECKED`, `LIVE_CHECKS_SKIPPED`,
  `CORRELATION_INSUFFICIENT`
- `GAP_RISK` says a stop does not protect against a gap and names position size as the real limit
- a suppressed model reading says to treat the number as **absent rather than neutral**

**Two real findings.**

1. **RSI was silently skipped whenever the window had no down-bars.** The guard was
   `if loss > 0`, so on a strongly trending instrument — precisely when an overbought reading is
   worth showing — the indicator and its convention bullet vanished entirely. A window with no
   losses is RSI 100, not "no RSI". Now handled explicitly at both ends.
2. **A test-data lesson worth keeping, because it is a real property of the measure.** Two
   monotonic price series moving in *opposite* directions still correlate at **+0.99** on
   returns, because both return series are monotonically decreasing (`3/1003, 3/1006, …` and
   `−3/2997, −3/2994, …`). Correlation measures co-movement of returns, not agreement of price
   direction. Genuine anti-correlation needs a mirrored step sequence. Both cases are now
   asserted, so nobody later "fixes" the correlation code to make opposite-trending symbols
   report negative.
...
`npm run audit` still reports **89** bridge calls, unchanged across Sections 74, 75 and 76,
because the audit counts calls *from* `.jsx` files and **no UI panel calls any of the thirteen
new methods**. The ledger, the alerts and the justification are all reachable by API only. That
is now the single largest gap in StockMind and the obvious next piece of work.
...
### Section 91 follow-up — the probe under-reported, and master hit it

Master, on his own machine: *"when checked for readiness, seeing limits python packages missing-fastapi."*

That message is the **in-range** branch, so his interpreter is fine — but the probe was wrong in a way
that would have cost him ten round trips.

**`import fastapi, uvicorn, pandas, ...` as a single statement aborts at the FIRST failure.** So on a
machine with nothing installed it reported only `fastapi`. Master installs fastapi, re-runs, is told
`uvicorn` is missing, and so on through the whole list. Three corrections:

1. **Each module is imported separately** and every failure is collected. The cost is identical —
   each module is imported once either way — and the report is complete. Verified on a venv holding
   only `fastapi`/`httpx`: it now names all ten remaining, where before it would have said `uvicorn`
   and stopped.
2. **All-missing and some-missing get different words.** Listing thirteen names when the answer is
   "nothing is installed in this interpreter" buries the instruction.
3. **A real import, not `importlib.util.find_spec`.** find_spec answers *is it installed*; the
   question is *will `main.py` start*. A wheel that installed but whose native binary is broken —
   the usual scipy/lightgbm failure — passes find_spec and fails on import.

The module list also moved to **import names, not distribution names**: `scikit-learn` imports as
`sklearn`, and checking the distribution name would report a working install as broken. It now covers
all thirteen the engine files import, not the ten first written.

**Two further defects found while testing this:**

**The probe ignored `RAMA_PYTHON`.** It checked bare `python` while `aiProcess.cjs` honours the
override — so master could point Rāma at a working venv, have the app satisfied, and still be told by
readiness that his packages were missing. **A diagnostic that contradicts the runtime is worse than
none**, because it sends master to fix something that is not broken. Both now resolve the interpreter
identically, and the output says `(via RAMA_PYTHON)` when an override is in force.

**The version check returned early**, so on an out-of-range interpreter master learned nothing about
whether the packages were there. It now reports the version problem and **continues** — "wrong Python,
and nothing installed in it either" is a different job from "wrong Python, otherwise ready". That also
removed a path that was unreachable on any machine whose only Python is out of range, which is the
machine this was written on: the loop could not be exercised here until the short-circuit went.

### Verified

Both branches exercised for real on this machine. All-missing: `Python 3.14.4 found, but NONE of the
engine packages are installed in it`. Partial: a throwaway venv with `fastapi` and `httpx` installed
reported `10 of 13 engine packages missing — uvicorn, numpy, pandas, scipy, sklearn, lightgbm,
xgboost, statsmodels, ta, joblib`. The configured-but-unstartable path was exercised too, by pointing
...
## SECTION 109 — Pop-outs become real windows, and the comments get shorter

Master: *"concise the comments overall"*, and *"do the above mentioned as best possible"* — the Section
108 plan. Day 1 of that plan is his machine, not mine, so this takes the highest-value row I can
actually finish and verify here: **pop-out windows that carry a session and dock back** (plan days
12–13), which he has now asked for twice.

### Pop-out session hand-off

The blocker was known since Section 100: `sessionStorage` is per-window, so a second `BrowserWindow`
starts unauthenticated and every gated channel correctly refuses it.

`electron/lib/popoutGrant.cjs` mints a **single-use ticket**: 32 random bytes, bound to
`(user, panel)`, main-process memory only, 30-second expiry, burned on first redemption whatever the
outcome. The pop-out redeems it once for the opener's user object and saves it to its own session.

**Two decisions worth keeping.**

**The ticket, not the token.** Putting the real session token on a URL leaves a long-lived credential in
histories, logs and crash reports. A ticket that dies on first use is worthless to anything that reads
it afterwards, and worthless before redemption to anything that is not that window.

**It creates no new authority.** The renderer already passes `user` to every gated channel, so handing
that same object to another window adds nothing it could not already do. `popoutGrant` makes **no
capability decision at all** — each channel still checks the user against `shared/capabilities.json`
per call (I1, I2, I8). The suite asserts this by grepping the module for capability logic.

**Docking back needs no ticket.** `popout:dock` revokes any unredeemed grant, tells the opener to
restore the panel, and destroys the window. `PanelBoard` removes a popped-out panel from the board and
shows it as a `⇤ TITLE` button in the toolbar — so a panel is never visible in two places at once, and
the way back does not require hunting for the window.

A pop-out only leaves the board if the window actually opened; otherwise the panel would vanish with
nowhere to be.

### Verified

`scripts/verifyPopoutGrant.cjs` — **39 assertions**, mostly about what must not work: replay from the
same window and from a different one, expiry at the boundary, a grant with no user or no id (it would
redeem into a session that looks authenticated and is not), a mutation of the live user object after
minting, a bounded map so renderer-triggered minting cannot exhaust memory, revocation when the window
closes, and `__proto__` as a ticket. Plus three source-level assertions: nothing is persisted, no
capability decision is made here, and the token comes from a CSPRNG.

`npm run verify` **19 suites**; audit clean; `vite build` succeeds.

**Not verified:** no shell was launched, so the hand-off is reasoned from the IPC path. The first real
test is master popping a panel out and seeing the book render instead of "not signed in".

### Comment concision

Master asked for it and he is right — several headers had become essays. The pass keeps the **decision
and the trap** and cuts the narration: `timeframes.js` 148 → 96 comment lines, `indicators.js` 103 → 74,
`positionMath.js` 67 → 58, plus the `PriceChart` and `PanelBoard` headers roughly halved.

The rule applied: a comment earns its length by recording a decision, a measurement, or a defect that
